...
<ComponentSection>
  <div class="mb-4 rounded-md border border-kumo-brand/20 bg-kumo-brand/5 p-4">
    <p class="text-sm text-kumo-strong">
      ...
      loading. Import from `@photon-ai/kumo-solid/code` to use.
    </p>
  </div>
...
## Overview
  ...
  <p>
    A Shiki-powered syntax highlighter with TextMate grammars, dual light/dark
    themes, and lazy loading. Exported from a separate entry point (
    `@photon-ai/kumo-solid/code` ) to avoid bundling Shiki for apps that don't
    need it.
  </p>
</ComponentSection>
...
```
  ...
  <div class="mt-4 rounded-md border border-kumo-warning/20 bg-kumo-warning/5 p-4">
    <p class="text-sm text-kumo-strong">
      <strong>Important:</strong> Do not import from the main
      `@photon-ai/kumo-solid` entry.
...
<p>
  CodeHighlighted ships a curated set of common Shiki languages. Only load the
  languages you need.
</p>
...
<ComponentExample demo="CodeHighlightedSolidDemo">
  <CodeHighlightedSolidDemo client:visible />
</ComponentExample>
...
## Server-Side Usage
  ...
  <p>
    For SSR frameworks such as SolidStart and Astro, use the server utilities
    to highlight at build time.
  </p>
    ...
    <table class="w-full text-sm">
      ...
      <tbody>
        ...
        <tr class="border-b border-kumo-hairline/50">
          ...
          <td class="py-2 pr-4"><code>Accessor&lt;boolean&gt;</code></td>
        ...
        <tr>
          ...
          <td class="py-2 pr-4"><code>Accessor&lt;Error | null&gt;</code></td>
