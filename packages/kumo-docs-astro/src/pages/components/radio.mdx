...
```tsx
import { Radio } from "@photon-ai/kumo-solid";
```

### Granular

```tsx
import { Radio } from "@photon-ai/kumo-solid/components/radio";
```
...
<p>
  The `label` prop on `Radio.Item` accepts JSX content, so you can embed icons,
