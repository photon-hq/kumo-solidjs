...
import { Select, Text, Badge } from "@photon-ai/kumo-solid";
  ...
  return (
    <div class="grid gap-4">
      <div class="flex items-center gap-3">
        <span class="w-10 text-sm text-kumo-subtle">xs</span>
        <Select
      ...
      <div class="flex items-center gap-3">
        <span class="w-10 text-sm text-kumo-subtle">sm</span>
        <Select
      ...
      <div class="flex items-center gap-3">
        <span class="w-10 text-sm text-kumo-subtle">base</span>
        <Select
      ...
      <div class="flex items-center gap-3">
        <span class="w-10 text-sm text-kumo-subtle">lg</span>
        <Select
...
export function SelectCustomRenderingDemo() {
  const [value, setValue] = createSignal(languages[0]);
...
export function SelectLoadingDataDemo() {
  ...
  const [data, setData] = createSignal<undefined | string[]>();
  const [value, setValue] = createSignal<string | null>(null);
...
export function SelectMultipleWithIndicatorDemo() {
  const [value, setValue] = createSignal<string[]>(["bug", "feature"]);
...
export function SelectComplexDemo() {
  const [value, setValue] = createSignal<(typeof authors)[0] | null>(null);
...
export function SelectDisabledOptionsDemo() {
  const [value, setValue] = createSignal<Region | null>(null);
  ...
  return (
    ...
    >
      {regions.map((region) => (
        <Select.Option value={region} disabled={region.disabled}>
          {region.label}
        </Select.Option>
      ))}
    </Select>
...
export function SelectDisabledItemsDemo() {
  const [value, setValue] = createSignal<string | null>("free");
    ...
    >
      <Select.Group>
        ...
        {foods.fruits.map((food) => (
          <Select.Option value={food}>{food.label}</Select.Option>
        ))}
      </Select.Group>
      <Select.Separator />
      <Select.Group>
...
export function SelectGroupedWithDisabledDemo() {
  const [value, setValue] = createSignal<ServerRegion | null>(null);
    ...
    >
      <Select.Group>
        ...
        {serverRegions.available.map((region) => (
          <Select.Option value={region}>{region.label}</Select.Option>
        ))}
      </Select.Group>
      <Select.Separator />
      <Select.Group>
        ...
        {serverRegions.unavailable.map((region) => (
          <Select.Option value={region} disabled>
            {region.label}
          </Select.Option>
        ))}
      </Select.Group>
...
export function SelectLongListDemo() {
  const [value, setValue] = createSignal<LongListItem | null>(null);
  ...
  return (
    ...
    >
      {longListItems.map((item) => (
        <Select.Option value={item}>{item.label}</Select.Option>
      ))}
    </Select>
  );
}
