...
export function CloudflareLogo(inputProps: CloudflareLogoProps) {
  const [props, svgProps] = splitProps(inputProps, [
    "variant",
    "color",
    "class",
    "className",
  ...
  const variant = () =>
    props.variant ?? KUMO_CLOUDFLARE_LOGO_DEFAULT_VARIANTS.variant;
  const color = () =>
    props.color ?? KUMO_CLOUDFLARE_LOGO_DEFAULT_VARIANTS.color;
  const fillOrange = () =>
    color() === "color" ? CLOUDFLARE_ORANGE : "currentColor";
  const fillYellow = () =>
    color() === "color" ? CLOUDFLARE_YELLOW : "currentColor";
  const colorClass = () =>
    cn(
      color() === "color" && variant() === "full" && "text-kumo-default",
    ...
    <>
      {variant() === "glyph" ? (
        ...
        >
          ...
          {CLOUDFLARE_WORDMARK_PATHS.map((path) => (
...
export interface PoweredByCloudflareProps extends Omit<
  AnchorProps,
  "class" | "className" | "color" | "href" | "ref"
> {
  color?: CloudflareLogoColor;
  href?: string;
  class?: string;
...
export function PoweredByCloudflare(inputProps: PoweredByCloudflareProps) {
  const [props, anchorProps] = splitProps(inputProps, [
    "color",
    "href",
    "class",
    "className",
  ...
  return (
    <a
      ref={props.ref}
      href={props.href ?? "https://www.cloudflare.com"}
      target="_blank"
      ...
      {...anchorProps}
    >
      <CloudflareLogo variant="glyph" color={color()} class="h-4 w-auto" />
      <span>
        Powered by <span class="font-semibold">Cloudflare</span>
      </span>
    </a>
  );
}
...
export function generateCloudflareLogoSvg(
  ...
  const wordmarkPaths = CLOUDFLARE_WORDMARK_PATHS.map(
    (path) => `  <path d="${path}" fill="${fillText}"/>`,
  ).join("\n");
